...
def simple_orthophoto(meta: ImageMetadata,
    ...
    # 픽셀 단위 초점거리 (센서 폭은 DJI 보급기 기준 13.2mm 가정 - 실제는 카메라별로 다름)
    sensor_width_mm = 13.2
    f_px = meta.focal_length * w_img / sensor_width_mm
    ...
    for px, py in corners_px:
        # 카메라 좌표계의 ray
        ray_cam = np.array([-(px - w_img / 2) / f_px,
                            -(py - h_img / 2) / f_px,
                            1.0])
        print('ray_cam', ray_cam)
        # 월드 좌표계로 회전
        ...
        if abs(ray_world[2]) < 1e-9:
            continue
        # Z=ground_z 평면과 교차
        t = (ground_z - camera_xyz[2]) / ray_world[2]
        print('ground_z', ground_z, 'camera_xyz[2]', camera_xyz[2], 'ray_world[2]', ray_world[2], 't', t)
        if t < 0:
            continue
        gx = camera_xyz[0] + t * ray_world[0]
        gy = camera_xyz[1] + t * ray_world[1]
        ground_corners.append([gx, gy])

    print(ground_corners, len(ground_corners))
    ...
    src_x[valid] = (w_img / 2) - f_px * R_diff[valid, 0] / den[valid]
    src_y[valid] = (h_img / 2) - f_px * R_diff[valid, 1] / den[valid]
...
def run_pipeline_gcp_free(image_dir: Path,
    ...
    print('cams_opt', cams_opt)

    # 6. 정사영상 생성 (사진별)
    ground_z_avg = np.mean([m.gps.altitude - (m.relative_height or 100.0) for m in metas])
    
    logger.info("추정 평균 지표 고도: %.2f m", ground_z_avg)
