...
사용:
    ...
    python scripts/run_auto_label.py \
...
def main() -> None:
    ...
    ap.add_argument("--strategy", default="heuristic",
                    choices=["heuristic", "sam2", "yolo_world", "yolo11n"])
    ap.add_argument("--classes",  nargs="+", default=["solar_panel"])
    ap.add_argument("--split",    type=float, nargs=3,
                    default=[0.7, 0.2, 0.1])
    ap.add_argument("--skip-auto-label", action="store_true",
                    help="자동 라벨링은 스킵하고 기존 데이터셋으로 다음 단계 진행")
