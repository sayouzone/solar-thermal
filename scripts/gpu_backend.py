...
이 모듈은 사용 가능한 GPU 라이브러리를 import-time 에 감지하여
런타임에 안전하게 분기할 수 있도록 plain 플래그/얇은 wrapper 만 제공한다.

감지 대상:
  - CuPy           : BA residual, triangulation, ortho 좌표 생성 가속
  - OpenCV CUDA    : SIFT(CUDA SIFT는 OpenCV 4.5.1+) / SURF / ORB + BFMatcher_CUDA, cuda.remap
  - PyTorch + CUDA : LightGlue 등 학습 기반 매칭 (선택)
...
try:
    ...
    if _torch.cuda.is_available():
        ...
        HAS_TORCH = True
        try:
            # LightGlue 는 별도 pip 패키지. 없으면 그냥 SIFT/ORB 폴백.
