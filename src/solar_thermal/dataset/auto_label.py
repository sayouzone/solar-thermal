...
class FineTunedYOLODetector:
    """파인튜닝된 일반 YOLO 검출기 (텍스트 프롬프트 없음)."""

    def __init__(
        self,
        class_id: int = 0,
        model_name: str = "models/best.pt",
        conf: float = 0.05,
        target_class_ids: Sequence[int] | None = None,  # 모델의 어떤 클래스를 solar로 볼지
    ):
        self.class_id = class_id
        self.conf = conf
        self.target_class_ids = set(target_class_ids) if target_class_ids else None
        ...
        for r in results:
            ...
            for (x1, y1, x2, y2), c in zip(xyxy, cls):
                if self.target_class_ids is not None and c not in self.target_class_ids:
                    continue
                bboxes.append(BBox.from_xyxy(x1, y1, x2, y2, w, h, self.class_id))
        return LabelResult(image_path, (h, w), bboxes)
...
def run(
    ...
    if strategy == "heuristic":
        ...
        detector = SAM2PanelDetector(class_id=class_id)
    ...
    elif strategy == "yolo11n":
        detector = FineTunedYOLODetector(class_id=class_id, model_name="models/best.pt", target_class_ids=("solar panel", "photovoltaic module"), conf=0.05)
    else:
